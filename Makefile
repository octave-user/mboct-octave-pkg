...
install_global: $(release_tarball)
	...
	$(OCTAVE) --eval 'pkg("install","-global","-verbose","$(release_tarball)");'

uninstall_local:
	@echo "Uninstalling package locally ..."
	$(OCTAVE) --eval '$(octave_pkg_prefix_local_cmd)pkg("uninstall","-local","-verbose","$(package)");'

uninstall_global:
	@echo "Uninstalling package globally ..."
	$(OCTAVE) --eval 'pkg("uninstall","-global","-verbose","$(package)");'
...
.PHONY: run doctest check uninstall_global uninstall_local
